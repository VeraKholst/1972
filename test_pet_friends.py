...
def test_create_pet_simple_successfully(name="Cat-3", animal_type="кот", age="4"):
...
def test_add_photo_to_pet_with_valid_data(pet_photo="images/cat.jpg"):
    ...
    if len(my_pets['pets']) == 0:
        pf.create_pet_simple(auth_key, "Cat-3", "кот", "3")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_get_api_key_for_invalid_user(email="unreal@mail.ru", password="unreal_password"):
...
def test_get_api_key_for_valid_user_invalid_password(email=valid_email, password="unreal_password"):
...
def test_get_api_key_for_invalid_user_valid_password(email="nikitinanika@mail.ru", password=valid_password):
...
def test_create_pet_simple_wrong_age(name="CatCat", animal_type="кот", age="очень много"):
    '''Тест 8: Добавление питомца (без фото) с неверно написанным возрастом. Баг. Возраст должен быть введен числом'''
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.create_pet_simple(auth_key, name, animal_type, age)
    assert status == 200
